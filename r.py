...
formatter = colorlog.ColoredFormatter(
    ...
    log_colors={
        'DEBUG': 'cyan',
        'INFO': 'green',      # Xanh lá cho [INFO]
        'WARNING': 'yellow',  # Vàng cho [WARNING]
        'ERROR': 'red',       # Đỏ cho [ERROR]
        'CRITICAL': 'red,bg_white',
    },
    secondary_log_colors={},  # Không áp dụng màu cho message
    style='%'
)
...
def format_log_message(message, variables=None, profit=None, section=None):
    formatted = message
    # Màu cho các section (CHỈ BÁO, THỊ TRƯỜNG, CHIẾN LƯỢC)
    if section:
...
async def train_advanced_model(ohlcv, historical_closes, historical_highs, historical_lows, initial=False):
    ...
    try:
        ...
        log_with_format('info', "Hoàn tất huấn luyện. Win Rate: {win_rate}", variables={'win_rate': f"{performance['win_rate']:.2%}"})
    except Exception as e:
...
async def get_historical_data():
    ...
    for attempt in range(3):
        try:
            ...
            if avg_volume < 1000:
                log_with_format('warning', "Thanh khoản thấp: Volume trung bình={volume} < 1000",
