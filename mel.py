...
async def place_order_with_tp_sl(side, price, quantity, volatility, predicted_price, atr):
    ...
    try:
        ...
        if side == 'buy':
            sl_price = entry_price * (1 - STOP_LOSS_PERCENT_ADJUSTED)
            tp_price = predicted_price if predicted_price and abs(predicted_price - entry_price) > min_price_diff else entry_price * (1 + TAKE_PROFIT_PERCENT_ADJUSTED)
        else:  # side == 'sell'
            sl_price = entry_price * (1 + STOP_LOSS_PERCENT_ADJUSTED)
            tp_price = predicted_price if predicted_price and abs(predicted_price - entry_price) > min_price_diff else entry_price * (1 - TAKE_PROFIT_PERCENT_ADJUSTED)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if sl_status['status'] == 'open':
                    ...
                    raise Exception("Lệnh SL không hợp lệ")
            except Exception as e:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if tp_status['status'] == 'open':
                    ...
                    raise Exception("Lệnh TP không hợp lệ")
            except Exception as e:
